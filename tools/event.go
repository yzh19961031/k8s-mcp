...
func RegisterEventTools(s *server.MCPServer, mgr ClusterManagerInterface) {
	s.AddTool(mcp.NewTool("get_events",
		...
		return mcp.NewToolResultText(getEventsImpl(ctx, client, cluster, namespace, limit)), nil
	})
}

func getEventsImpl(ctx context.Context, client kubernetes.Interface, cluster, namespace string, limit int) string {
